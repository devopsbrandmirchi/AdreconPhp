...
const ALLOWED_INTERVALS = [0, 1, 3, 6, 12, 24, 168];
const ONCE = 0;
...
/** The same thing phrased to follow the word "checked". */
function interval_phrase(int $hours): string {
    return match (true) {
        $hours === ONCE => 'once, then stops',
        $hours === 1    => 'every hour',
        $hours === 24   => 'daily',
        $hours === 168  => 'weekly',
...
function run_tracker(array $tracker, string $trigger = 'scheduled'): array {
    ...
    // A queued provider posts the task and stops here. The schedule is moved
    // on straight away so the same check cannot be posted twice while it waits.
    if (provider_is_async()) {
        $pdo->prepare("UPDATE runs SET status = 'queued' WHERE id = ?")->execute([$runId]);
        advance_schedule($tracker);

        $posted = provider_queue_check($tracker, $runId);
        if (!$posted['ok']) {
            $pdo->prepare(
                ...
                $posted['http'], substr((string)$posted['error'], 0, 500),
                $posted['raw'] ? substr((string)$posted['raw'], 0, 200000) : null, $runId,
            ]);
            bump_failure($tracker);
            return [false, (string)$posted['error']];
        }

        // A very fast provider can push the result back before this line runs,
        // so only fill in what is still missing rather than resetting the row.
        $pdo->prepare(
            "UPDATE runs SET task_id = COALESCE(NULLIF(task_id, ''), ?), credits_used = ?
             WHERE id = ?"
        )->execute([$posted['task_id'], provider_cost(), $runId]);

        $st = $pdo->prepare('SELECT status FROM runs WHERE id = ?');
        $st->execute([$runId]);
        $now = (string)$st->fetchColumn();

        return [true, $now === 'queued' ? 'queued, results follow shortly' : 'queued and already back'];
    }
    ...
    $msg = finish_run($runId, $tracker, $ads, $meta, (string)$res['raw']);
    advance_schedule($tracker);   // a live provider is finished, so move it on now
    return [true, $msg];
}

/**
 * Write a finished check: counts, placements, and the schedule's next slot.
 *
 * Shared by the synchronous providers and by DataForSEO's queue, so a result
 * that arrives minutes later is stored in exactly the same shape as one that
 * arrived immediately.
 */
function finish_run(int $runId, array $tracker, array $ads, array $meta, ?string $raw): string {
    $pdo = db();
    ...
    $pdo->prepare(
        ...
        $raw !== null ? substr($raw, 0, 200000) : null, $runId,
    ]);

    $pdo->prepare('DELETE FROM ad_placements WHERE run_id = ?')->execute([$runId]);
    ...
    // Only the result is recorded here. Whoever called this decides when the
    // schedule moves on: immediately for a live provider, at post time for a
    // queued one.
    $pdo->prepare(
        "UPDATE trackers SET last_run_at = UTC_TIMESTAMP(), fail_streak = 0 WHERE id = ?"
    )->execute([$tracker['id']]);
...
/** Alias with a name that reads properly from the queued path. */
function mark_tracker_failed(array $tracker): void {
    bump_failure($tracker);
}

/** Move a schedule to its next slot without touching its results. */
function advance_schedule(array $tracker): void {
    if ((int)$tracker['interval_hours'] === ONCE) {
        db()->prepare("UPDATE trackers SET next_run_at = NULL, status = 'done' WHERE id = ?")
            ->execute([$tracker['id']]);
        return;
    }
    db()->prepare('UPDATE trackers SET next_run_at = ? WHERE id = ?')
 ...
 */
function reap_queued_runs(): int {
    $hours = max(1, (int)cfg('queued_timeout_hours', 6));
    $stmt  = db()->prepare(
        "UPDATE runs
         SET status = 'failed', finished_at = UTC_TIMESTAMP(),
             error_message = 'The provider never returned this queued check'
         WHERE status = 'queued' AND started_at < DATE_SUB(UTC_TIMESTAMP(), INTERVAL ? HOUR)"
    );
    $stmt->execute([$hours]);
    return $stmt->rowCount();
}
